...
const TASK_CONFIG: Record<string, TaskConfig> = {
  ...
  strategy:   { model: "anthropic/claude-sonnet-4-6", provider: "openrouter", fallbackModel: "gemini-2.5-pro", fallbackProvider: "gemini" },
  ...
  image:      { model: "google/gemini-3.1-flash-image-preview", provider: "openrouter" },
  image_hq:   { model: "google/gemini-3.1-flash-image-preview", provider: "openrouter" },
  analyze:    { model: "anthropic/claude-sonnet-4-6", provider: "openrouter", fallbackModel: "gemini-2.5-flash", fallbackProvider: "gemini" },
  tag_image:  { model: "gemini-2.5-flash", provider: "gemini", fallbackModel: "anthropic/claude-sonnet-4-6", fallbackProvider: "openrouter" },
  video:      { model: "anthropic/claude-sonnet-4-6", provider: "openrouter", fallbackModel: "gemini-2.5-pro", fallbackProvider: "gemini" },
...
const COST_MAP: Record<string, { input: number; output: number }> = {
  ...
  "google/gemini-3.1-flash-image-preview": { input: 2, output: 12 },
...
async function callProvider(
  ...
  // Gemini API (OpenAI-compatible endpoint — requires Bearer auth)
...
serve(async (req) => {
  ...
  try {
    ...
    // Accept internal calls (from other edge functions) via INTERNAL_SECRET or service role key
    const isInternalCall = (internalSecret && token === internalSecret) || (serviceRoleKey && token === serviceRoleKey);
