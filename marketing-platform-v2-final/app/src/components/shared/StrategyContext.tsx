...
import { Brain, ChevronDown, ChevronUp, Target, MessageSquare, Users, Zap, Loader2 } from 'lucide-react';
import { useStrategyContext } from '@/hooks/useWorkspace';
...
const SECTIONS: ContextSection[] = [
  { id: 'brand', icon: Target, label: 'Posicionamento', color: 'text-brand' },
  { id: 'persona', icon: Users, label: 'Persona', color: 'text-teal' },
  { id: 'tone', icon: MessageSquare, label: 'Tom de Voz', color: 'text-amber-400' },
  { id: 'differentials', icon: Zap, label: 'Diferenciais', color: 'text-emerald-400' },
];

export function StrategyContext({ onInjectContext }: StrategyContextProps) {
  const [expanded, setExpanded] = useState(false);
  const [activeFields, setActiveFields] = useState<Set<string>>(new Set());
  const { context, hasStrategy, isLoading } = useStrategyContext();
          ...
          {!hasStrategy ? (
            ...
            <>
              ...
              {activeFields.size > 0 && onInjectContext && (
                <button
                  onClick={() => onInjectContext(context)}
