...
import { Upload, Search, Image, Filter, X, Tag, Loader2, Trash2 } from 'lucide-react';
import type { MediaItem, MediaMood } from '@/types';
import { cn } from '@/lib/utils';
import { useMediaItems, useUploadMedia, useDeleteMedia, type MediaFilters } from './hooks/useMediaLibrary';
...
export function MediaLibrary() {
  ...
  const [moodFilter, setMoodFilter] = useState<MediaMood | ''>('');
  ...
  return (
    <div className="flex gap-6 h-full">
      ...
      <div className="w-64 shrink-0 space-y-5 overflow-y-auto pr-2">
        ...
        <div
          className={cn(
            'border-2 border-dashed border-border rounded-xl p-6 text-center hover:border-brand/50 transition-colors cursor-pointer',
        ...
        >
          {uploadMutation.isPending ? (
            <Loader2 className="w-8 h-8 mx-auto text-brand animate-spin mb-2" />
          ) : (
            <Upload className="w-8 h-8 mx-auto text-text-muted mb-2" />
          )}
          <p className="text-xs text-text-muted">
            {uploadMutation.isPending ? 'Enviando...' : 'Arraste imagens ou clique'}
          </p>
          <input
            id="file-upload"
            type="file"
              ...
              >
                ...
                {item.tagging_status === 'done' && (
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {item.ai_mood && (
                      <span className="text-[9px] bg-white/20 text-white px-1.5 py-0.5 rounded mt-1 inline-block capitalize">{item.ai_mood}</span>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="fixed right-0 top-0 h-screen w-80 bg-surface-elevated border-l border-border p-5 space-y-4 overflow-y-auto z-50">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={() => { deleteMutation.mutate(selectedItem.id); setSelectedItem(null); }}
                  className="text-text-muted hover:text-danger p-1"
                  title="Excluir"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
                <button onClick={() => setSelectedItem(null)} className="text-text-muted hover:text-text-primary p-1">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {selectedItem.ai_fit_score_map && Object.keys(selectedItem.ai_fit_score_map).length > 0 && (
                <div>
