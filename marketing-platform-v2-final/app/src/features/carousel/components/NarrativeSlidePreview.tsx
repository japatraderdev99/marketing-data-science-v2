...
import { NARRATIVE_PALETTE, Body, DqMark, HeadlineRenderer } from './NarrativeSlideHelpers';
...
export const NarrativeSlidePreview = forwardRef<HTMLDivElement, Props>(
  ({ slide, theme, width = 340, height = 425, imageUrl, settings, totalSlides, isExport = false }, ref) => {
    ...
    const hlX = settings?.headlineX ?? 0; const hlY = settings?.headlineY ?? 0;
    const bdX = settings?.bodyX ?? 0; const bdY = settings?.bodyY ?? 0;
    ...
    const imgBaseStyle: React.CSSProperties = {
      ...
      objectFit: 'cover', zIndex: 0, opacity: imgOpacity,
      ...
      return (
        <div ref={ref} style={{ width, height, overflow: 'hidden', position: 'relative', flexShrink: 0, borderRadius: isExport ? 0 : 12 }}>
          <div className="slide-export" style={{
            ...
            display: 'flex', flexDirection: 'column', padding: '108px 88px 100px',
          }}>
            ...
            <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              {slide.bodyText && (
                <Body text={slide.bodyText} accent={cardAccent} color={bodyColor} size={bdSize} offsetX={bdX} offsetY={bdY} />
              )}
              {slide.sourceLabel && (
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', marginTop: 48 }}>
                <div style={{
                  width: '100%', height: 480, borderRadius: 24, overflow: 'hidden', position: 'relative',
                  background: isDark
    ...
    return (
      <div ref={ref} style={{ width, height, overflow: 'hidden', position: 'relative', flexShrink: 0, borderRadius: isExport ? 0 : 12 }}>
        <div className="slide-export" style={{
          ...
          {hasPhoto && <img src={imageUrl!} alt="" style={imgBaseStyle} />}
          {isPhotoFull && (
            <div style={{
